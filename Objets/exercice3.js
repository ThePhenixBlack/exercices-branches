...
for (let i=0 ; i< data.products.length ; i++) 
{
  ...
  if(data.products[i].rating > 3){
        film.push(data.products[i].title)
   descriptions.push(data.products[i].description)
    note.push(data.products[i].rating)
  }
}

console.log(film,descriptions,note)
console.log(`Il y a ${(film.length)} film avec une moyenne de plus de 3 `)
console.log(`Les films sont ${film}`)
console.log(`Les notes sont ${note}`)